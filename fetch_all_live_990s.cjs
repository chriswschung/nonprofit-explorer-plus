...
const searchQueries = [
  'wildlife', 'nature', 'hospital', 'medical', 'university', 'college',
  'humanitarian', 'relief', 'food bank', 'shelter', 'museum', 'art',
  'civil rights', 'justice', 'child', 'youth', 'animal', 'humane',
  'science', 'research', 'red cross', 'salvation army', 'united way',
  'goodwill', 'cancer', 'heart', 'habitat', 'feeding', 'ymca', 'ywca'
];
...
async function main() {
  console.log('Fetching live ProPublica Form 990 data in parallel batch mode...');
  const orgSummaries = new Map();

  const searchPromises = searchQueries.map(async (query) => {
    const searchRes = await fetchJson(`https://projects.propublica.org/nonprofits/api/v2/search.json?q=${encodeURIComponent(query)}`);
    if (searchRes && searchRes.organizations) {
      for (const org of searchRes.organizations) {
        if (org.ein) orgSummaries.set(org.ein, org);
      }
    }
  });

  await Promise.all(searchPromises);
  console.log(`Discovered ${orgSummaries.size} unique organizations across sectors.`);

  const orgEntries = Array.from(orgSummaries.entries());
  const compiledNonProfits = [];
  const BATCH_SIZE = 15;

  for (let i = 0; i < orgEntries.length && compiledNonProfits.length < 300; i += BATCH_SIZE) {
    ...
    console.log(`Progress: Compiled ${compiledNonProfits.length} live Form 990 non-profits...`);
  }

  console.log(`\nSuccessfully compiled ${compiledNonProfits.length} live Form 990 records!`);
